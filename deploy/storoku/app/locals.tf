locals {
  ...
  cache_user          = local.dedicated_resources ? length(var.caches) > 0 ? module.caches[0].iam_user : null : try(data.terraform_remote_state.shared.outputs.dev_caches.iam_user, {})
  ...
  domain = {
    # hostname overrides the derived name; staging in its own account owns a
    # zone whose apex is the service hostname, so the "staging." prefix is wrong there
    name    = var.hostname != "" ? var.hostname : local.is_production ? local.domain_base : local.is_staging ? "staging.${local.domain_base}" : "${var.environment}.${local.domain_base}"
    zone_id = data.terraform_remote_state.shared.outputs.route53_zones[var.network].zone_id
  }
  # DID and principal mapping are only meaningful for a service that has an identity
