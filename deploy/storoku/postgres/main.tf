...
resource "aws_security_group" "rds" {
  ...
  ingress {
    ...
    to_port     = 5432
    protocol    = "tcp"
    cidr_blocks = flatten([var.vpc.private_cidr_blocks, var.vpc.db_cidr_blocks, var.vpc.public_cidr_blocks])
  }

  // ingress bastion host
  dynamic "ingress" {
    for_each = var.db_config.bastion ? [1] : []
    content {
      from_port   = 5432
      to_port     = 5432
      protocol    = "tcp"
      description = "Bastion"
      cidr_blocks = ["${aws_instance.database_bastion_host[0].private_ip}/32"]
    }
  }
...
resource "aws_db_instance" "rds" {
  ...
  final_snapshot_identifier           = "${var.environment}-${var.app}-final"
...
resource "aws_security_group" "bastion_host" {
  count       = var.db_config.bastion ? 1 : 0
...
resource "aws_eip" "bastion-host-eip" {
  count    = var.db_config.bastion ? 1 : 0
  instance = aws_instance.database_bastion_host[0].id
  tags = {
