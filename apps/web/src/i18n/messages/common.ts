...
export const commonMessages = defineMessages({
  ...
  "cp.common.emptyBody": {
    ...
    de: "Datensätze erscheinen hier, sobald an diesem Konto gearbeitet wird.",
  ...
  "cp.common.noMatchTitle": {
    ...
    de: "Keine Datensätze für diese Filter",
  ...
  "cp.common.permissionBody": {
    ...
    ja: "必要なアクセス権の付与をアカウント所有者に依頼してください。",
    pt: "Peça a um proprietário da conta que conceda o acesso necessário.",
    zh: "请联系账户所有者授予所需权限。",
  ...
  "cp.common.freshnessStaleBody": {
    ...
    de: "Die Daten dieser Seite sind noch nicht mit ihrer Quelle synchronisiert; eine aktuelle Änderung kann fehlen. Aktualisieren Sie die Seite, bevor Sie hier etwas veranlassen.",
    ja: "このページのデータは元の情報にまだ追いついておらず、最近の変更が反映されていない可能性があります。操作する前に再読み込みしてください。",
    pt: "Os dados desta página ainda não foram sincronizados com a origem, e uma alteração recente pode não aparecer. Atualize antes de agir com base neles.",
  ...
  "cp.common.freshnessPartialTitle": {
    ...
    de: "Diese Liste konnte nur teilweise abgerufen werden.",
  ...
  "cp.common.freshnessPartialBody": {
    ...
    de: "Dieses Konto enthält in diesem Kanal mehr Datensätze, als ein einzelner Abruf zurückgibt. Unten stehen die zuletzt aktualisierten Datensätze. Trefferzahl, Filteroptionen, Summen und Sortierung beziehen sich nur auf die abgerufenen Datensätze: Eine korrekt nach Betrag sortierte Liste kann trotzdem den größten Datensatz dieses Kontos nicht enthalten. Der unvollständige Abruf wurde gemeldet; erneutes Laden liefert die übrigen Datensätze nicht.",
    ja: "このアカウントには、一度に取得できる件数を超えるレコードがこのチャネルにあります。以下に表示されているのは、最近更新されたレコードです。件数、フィルターの選択肢、合計、並べ替えは、取得済みのレコードのみを対象とします。金額順に正しく並んでいても、このアカウントで最大のレコードが含まれていない場合があります。取得が不完全であることは報告済みです。再読み込みしても残りは取得されません。",
  ...
  "cp.common.unsavedBody": {
    ...
    de: "Die Eingaben in diesem Formular wurden noch nicht an den Server gesendet. Beim Verlassen gehen alle Eingaben verloren.",
  ...
  "settings.error": {
    ...
    ja: "サポートされている言語を選択して、もう一度お試しください。",
  ...
  "state.fatal.description": {
    ...
    de: "Laden Sie die Seite neu und prüfen Sie den aktuellen Datensatz, bevor Sie es erneut versuchen. Besteht das Problem weiterhin, wenden Sie sich mit der Anfrage-ID an den Support.",
    ja: "再試行する前にページを再読み込みし、最新のレコードを確認してください。問題が解決しない場合は、リクエスト ID を添えてサポートにお問い合わせください。",
