...
function mutation(options: ExternalGateMutationOptions) {
  ...
  if (!csrfToken || csrfToken.length < 32)
    throw new ExternalGateClientError(
      403,
      "Recent authentication and a secure form token are required.",
    );
  return {
...
function operationError(status: number): ExternalGateClientError {
  if (status === 409)
    return new ExternalGateClientError(
      status,
      "This gate changed on the server. Reload before retrying.",
    );
  if (status === 403)
    return new ExternalGateClientError(
      status,
      "Your authority or recent authentication could not be verified.",
    );
  if (status === 422)
    return new ExternalGateClientError(
      status,
      "Activation policy denied this change. Configuration alone cannot activate a gate.",
    );
  return new ExternalGateClientError(
    status,
