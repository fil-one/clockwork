...
export class DatabaseDeadLetterRecoveryStore {
   ...
   *
   * Only the outbox has a self-service retry: clearing its attempt ceiling puts
   * the message back under the dispatcher's own claim query. The other two
   * shapes are re-invoked by the caller through the lifecycle redrive, so this
   * records the request and leaves the record where the operator can see the
   ...
   * the same work concurrently, and refuses anything already abandoned.
   */
   ...
   *
   * The outbox owns its own redelivery, so clearing the ceiling is the whole
   * retry. A provisioning attempt and a workflow run are re-invoked by their
   * task runner, so their rows move out of the terminal state and the caller
   * supplies the redrive.
   */
